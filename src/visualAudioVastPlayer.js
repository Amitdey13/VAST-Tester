...
import { warn, xmlToJSON, parseXmlOrReturnUrl } from "./utils";
...
class VisualAudioVastPlayer {
    ...
    async loadVastData() {
        try {            
            ...
            this.videoSrc = this.getMediaFileUrl(xmlToJSON(this.getVideoLinearFromInLine(this.getAdFromVast(_vastType, warn, this.parsedXmlData[0]), warn)).mediaFiles);
    ...
    getVideoLinearFromInLine(inLine, warn) {
        const linearCreatives = inLine.creatives
            .toArray()
            .filter(creative => creative.linear != null)
        if (linearCreatives.length > 1) {
    ...
    validateParameters() {
        ...
        const notSpecified = Object.keys(requiredParams).filter(key => !requiredParams[key]);
        if (notSpecified.length) {
            throw new Error(`Parameters missing ${notSpecified.join(', ')}`)
        }
    }
